...
public class OverheadMazePanel extends JPanel implements MazeListener {
	...
	/**
	 * Create a Panel that will display an overhead view of a {@link Maze} from
	 * a specified {@link Client}'s viewpoint.
	 * 
	 * @param maze
	...
	public void paintComponent(Graphics g) {
		...
		if ((panelSize == null) || (panelSize != getSize())) {
			...
			height = (double) panelSize.height - (double) panelSize.height
					* 0.05;
			...
			player = new Arc2D.Double(new Rectangle2D.Double(-diameter / 2.0,
					-diameter / 2.0, diameter, diameter), 30.0, 300.0,
					Arc2D.PIE);
			diameter = java.lang.Math.min(cellwidth, cellheight) * 0.30;
			projectile = new Arc2D.Double(new Rectangle2D.Double(
					-diameter / 2.0, -diameter / 2.0, diameter, diameter), 0.0,
					360.0, Arc2D.PIE);
		}
		...
		for (int i = 0; i < p.getY(); i++) {
			for (int j = 0; j < p.getX(); j++) {
				...
				Line2D visLine = new Line2D.Double(llx + (cp.getX() + 0.5)
						* cellwidth, lly + (cp.getY() + 0.5) * cellheight, llx
						+ (j + 0.5) * cellwidth, lly + (i + 0.5) * cellheight);

				/* Visibility testing */
				/*
				 * Iterator visIt = wallList.iterator(); while(visIt.hasNext())
				 * { Object o = visIt.next(); if(o instanceof Line2D) { Line2D l
				 * = (Line2D)o; if(l.intersectsLine(visLine)) { cellVisible =
				 * false; } } else { throw new Error(); }
				 * 
				 * }
				 */
				if (cellVisible) {
					...
					if (o != null) {
						if (o instanceof Client) {
							...
							double xoffset = llx + j * cellwidth
									+ (cellwidth / 2.0);
							double yoffset = lly + i * cellheight
									+ (cellheight / 2.0);
							...
							GlyphVector name = font.createGlyphVector(frc,
									c.getName());
							g2.scale(1.0, -1.0);
							g2.setColor(Color.black);
		...
		for (int i = 0; i < p.getY(); i++) {
			for (int j = 0; j < p.getX(); j++) {
				...
				if (cell.isWall(Direction.North)) {
					wallList.add(new Line2D.Double(x + j * width, y + (i + 1)
							* height, x + (j + 1) * width, y + (i + 1) * height));
				}
				if (cell.isWall(Direction.East)) {
					wallList.add(new Line2D.Double(x + (j + 1) * width, y + i
							* height, x + (j + 1) * width, y + (i + 1) * height));
				}
			}
		}
