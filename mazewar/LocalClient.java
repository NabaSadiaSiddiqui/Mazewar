 ...
 * computer the game is running upon. You may choose to implement some of your
 * code for communicating with other implementations by overriding methods in
 * {@link Client} here to intercept upcalls by {@link GUIClient} and
...
public abstract class LocalClient extends Client {
	...
	protected void respawn(String name, Point point, Direction d) {
		assert (name.equals(Mazewar.guiClient.getName()));
		Mazewar.consolePrintLn("Respawn local client");

		/*
		 * Iterator clients = Mazewar.maze.getClients(); Client target = null;
		 * while(clients.hasNext()) { target = (Client) clients.next();
		 * if(target.getName().equals(ClientState.PLAYER_NAME)) {
		 * Mazewar.maze.removeClient(target); break; } }
		 */
