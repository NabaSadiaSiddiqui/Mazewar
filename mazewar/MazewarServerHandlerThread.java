	...
	public void run() {
		...
		try {
			...
			while (!gotQuitPacket
				 ...
				 * String playerName = packetFromClient.player; int playerAction
				 * = packetFromClient.action; int seq = ServerState.time;
				 * 
				 * SharedData.ActionInfo action;
				 * 
				 * if(playerAction == MazewarPacket.CLIENT_RESPAWN) { action =
				 * new SharedData.ActionInfo(playerName, playerAction, seq,
				 * packetFromClient.playerInfo); } else { action = new
				 * SharedData.ActionInfo(playerName, playerAction, seq);
				 * 
				 * if(playerAction == MazewarPacket.CLIENT_QUIT) { // Remove
				 * player from BlockingQueue<PlayerMeta> players Iterator
				 * players = ServerState.players.iterator(); PlayerMeta target =
				 * null; Point point = null; while(players.hasNext()) {
				 * PlayerMeta tmp = (PlayerMeta) players.next();
				 * if(tmp.name.equals(playerName)) { target = tmp; point = new
				 * Point(tmp.posX, tmp.posY); break; } }
				 ...
				 * 
				 * // increment logical time ServerState.time++;
				 * 
				 * continue; }
				 */
