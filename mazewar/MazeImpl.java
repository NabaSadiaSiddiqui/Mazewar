...
public class MazeImpl extends Maze implements Serializable, ClientListener,
	...
	private synchronized void killClient(Client source, Client target) {
		...
		if (target.getName().equals(Mazewar.guiClient.getName())) { // GUIClient
			...
			while (isCurrPosition(point)) {
				point = new Point(randomGen.nextInt(maxX),
						randomGen.nextInt(maxY));
			}
	...
	/**
	 * Obtain the {@link Point} which is not already occupied by other clients
	 * (remote and local)
	 * 
	 * @param occupiedCells
	 *            All points on the {@link maze} which are filled
	 ...
	 * 
	 * @param occupiedCells
	 *            The array of cells which are occupied by other players
	 * @param point
	 *            A potentially empty {@link Point}
	 ...
	 *            The location the {@link Client} should be added.
	 * @param direction
	 *            The direction the {@link Client} should be facing
	 */
	public synchronized void addClientAtPointWithDirection(Client client,
